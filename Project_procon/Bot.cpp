...
vector<ValueIndex*> Bot::getListValues(int index) {
    ...
    for (int i = 0; i < sizeOfDomain; i++) { // Tìm trong tất cả các Piece chưa ghép
        ...
        for (int j = 0; j < currPiece->getNumVertices(); j++) { // Tìm trong tất cả các đỉnh của Piece
            ...
            if (this->currAnswer->isPossibleToFill(index, currPiece, j)) { // Nếu lắp được tại index (không flip)
            ...
            if (this->currAnswer->isPossibleToFillFlip(index, currPiece, j) && FLIP_MODE) { // Nếu lắp được tại index mà có flip
